import { CancelledTransactionDataInterface } from "../../../interfaces/data.interfaces.js";

export const cancelledTransactions: CancelledTransactionDataInterface[] = [
  // User 1 transactions
  {
    user_id: "655b50b42e2bcd090b435230",
    name: "Rent Payment",
    type: "Direct Debit",
    frequency: {
      period: "monthly",
      custom_days: 0,
    },
    created_at: 1602828180000,
    amount: 1200000,
    history: [],
    cancelled_at: 1648272000000, 
  },
  {
    user_id: "655b50b42e2bcd090b435230",
    name: "Netflix Subscription",
    type: "Standing Order",
    frequency: {
      period: "monthly",
      custom_days: 0,
    },
    created_at: 1615148180000,
    amount: 1000,
    history: [],
    cancelled_at: 1646985600000, 
  },
  {
    user_id: "655b50b42e2bcd090b435230",
    name: "Electricity Bill",
    type: "Direct Debit",
    frequency: {
      period: "monthly",
      custom_days: 0,
    },
    created_at: 1627468180000,
    amount: 80000,
    history: [
      {
        amount: 1100000,
        created_at: 1615148180000,
      },
    ],
    cancelled_at: 1646985600000, 
  },
  {
    user_id: "655b50b42e2bcd090b435230",
    name: "Bouldering membership",
    type: "Recurring Payment",
    frequency: {
      period: "weekly",
      custom_days: 0,
    },
    created_at: 1639788180000,
    amount: 2000,
    history: [],
    cancelled_at: 1645756800000, 
  },
  {
    user_id: "655b50b42e2bcd090b435230",
    name: "Water Bill",
    type: "Recurring Payment",
    frequency: {
      period: "monthly",
      custom_days: 0,
    },
    created_at: 1639788180000,
    amount: 2500,
    history: [
      {
        amount: 28000,
        created_at: 1627468180000
      },
    ],
    cancelled_at: 1648272000000, 
  },

  // User 2 transactions
  {
    user_id: "655b5158c6965d869180e906",
    name: "Credit Card Payment",
    type: "Direct Debit",
    frequency: {
      period: "monthly",
      custom_days: 0,
    },
    created_at: 1609090180000,
    amount: 100000,
    history: [],
    cancelled_at: 1651420800000, 
  },
  {
    user_id: "655b5158c6965d869180e906",
    name: "Wine subscription",
    type: "Standing Order",
    frequency: {
      period: "monthly",
      custom_days: 0,
    },
    created_at: 1616770180000,
    amount: 5000,
    history: [],
    cancelled_at: 1650038400000, 
  },
  {
    user_id: "655b5158c6965d869180e906",
    name: "Life Insurance",
    type: "Direct Debit",
    frequency: {
      period: "quarterly",
      custom_days: 0,
    },
    created_at: 1624340180000,
    amount: 30000,
    history: [],
    cancelled_at: 1648272000000, 
  },
  {
    user_id: "655b5158c6965d869180e906",
    name: "Xbox Live",
    type: "Recurring Payment",
    frequency: {
      period: "monthly",
      custom_days: 0,
    },
    created_at: 1632020180000,
    amount: 800,
    history: [],
    cancelled_at: 1646985600000, 
  },

  // User 3 transactions
  {
    user_id: "655b51a746341227e519c2dc",
    name: "Savings",
    type: "Standing Order",
    frequency: {
      period: "monthly",
      custom_days: 0,
    },
    created_at: 1600912180000,
    amount: 20000,
    history: [],
    cancelled_at: 1645756800000, 
  },
  {
    user_id: "655b51a746341227e519c2dc",
    name: "Cineworld Card",
    type: "Direct Debit",
    frequency: {
      period: "monthly",
      custom_days: 0,
    },
    created_at: 1613232180000,
    amount: 2000,
    history: [],
    cancelled_at: 1648272000000, 
  },
  {
    user_id: "655b51a746341227e519c2dc",
    name: "Boxing Classes",
    type: "Direct Debit",
    frequency: {
      period: "quarterly",
      custom_days: 0,
    },
    created_at: 1625552180000,
    amount: 25000,
    history: [],
    cancelled_at: 1645756800000, 
  },
  {
    user_id: "655b51a746341227e519c2dc",
    name: "Veg box",
    type: "Recurring Payment",
    frequency: {
      period: "weekly",
      custom_days: 0,
    },
    created_at: 1637872180000,
    amount: 1000,
    history: [],
    cancelled_at: 1650038400000, 
  },

  // User 4 transactions
  {
    user_id: "655b5292f8a18265e0b77848",
    name: "Pet insurance",
    type: "Direct Debit",
    frequency: {
      period: "monthly",
      custom_days: 0,
    },
    created_at: 1605004180000,
    amount: 8500,
    history: [],
    cancelled_at: 1650038400000, 
  },
  {
    user_id: "655b5292f8a18265e0b77848",
    name: "Retirement",
    type: "Direct Debit",
    frequency: {
      period: "biannual",
      custom_days: 0,
    },
    created_at: 1617324180000,
    amount: 40000,
    history: [],
    cancelled_at: 1650038400000, 
  },
  {
    user_id: "655b5292f8a18265e0b77848",
    name: "Savings Pot",
    type: "Standing Order",
    frequency: {
      period: "monthly",
      custom_days: 0,
    },
    created_at: 1629644180000,
    amount: 30000,
    history: [
      {
        amount: 35000,
        created_at: 1617324180000
      },
    ],
    cancelled_at: 1650038400000, 
  },
];
